...
type t = { main : elt; target : elt option }

let superuser : t = { main = (0, [ READ; WRITE ]); target = None }
let of_domain domid : t = { main = (domid, [ READ; WRITE ]); target = None }
...
let is_dom0 (connection : t) = is_owner connection 0
...
let has (t : t) _p = if not (is_dom0 t) then raise Permission_denied
...
let check_owner (connection : t) (node : Xs_protocol.ACL.t) =
  if not (is_dom0 connection) then
    is_owner connection node.Xs_protocol.ACL.owner
  else true
...
let check (connection : t) request (node : Xs_protocol.ACL.t) =
  let check_acl domainid =
    let perm =
      if List.mem_assoc domainid node.Xs_protocol.ACL.acl then
        List.assoc domainid node.Xs_protocol.ACL.acl
