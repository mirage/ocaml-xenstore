...
let create max_elements =
  ...
  let push line =
    if !(!elements) > max_elements then incr !dropped_elements
    else (
...
let string_of_tid ~con tid =
  if tid = 0l then sprintf "%-12s" con
...
let string_of_access_type = function
  ...
  | Response (r, info_opt) -> (
      " -> out  "
      ^ Xs_protocol.Response.prettyprint_payload r
      ^ match info_opt with Some x -> " (" ^ x ^ ")" | None -> "")
...
let access_type_disabled = function
  ...
  | Debug _ -> false
  | Start_transaction | End_transaction -> !disable_transaction
  | Request r ->
      List.mem
        Xs_protocol.(Op.to_string (Request.ty_of_payload r))
        !disable_request
  | Response (r, _) -> (
      match r with
      | Xs_protocol.Response.Error x -> List.mem x !disable_reply_err
      | _ ->
...
let sanitize_data data =
  ...
  for i = 0 to Bytes.length data - 1 do
    if Bytes.get data i = '\000' then Bytes.set data i ' '
  done;
  String.escaped (Bytes.to_string data)

let access_logging ~con ~tid ?(data = "") access_type =
  if access_type_enabled access_type then
...
let read_coalesce ~tid ~con data =
  access_logging Coalesce ~tid ~con ~data:("read " ^ data)
...
let response ~tid ~con ?info response =
  access_logging ~tid ~con (Response (response, info))
