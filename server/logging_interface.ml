...
let read _t (perms : Perms.t) (path : Store.Path.t) =
  ...
  | "reply-err" :: [] -> ""
  | [ "request"; x ] ->
      if List.mem x !disable_request then "1" else Store.Path.doesnt_exist path
  | [ "reply-ok"; x ] ->
      if List.mem x !disable_reply_ok then "1" else Store.Path.doesnt_exist path
  | [ "reply-err"; x ] ->
      if List.mem x !disable_reply_err then "1"
      else Store.Path.doesnt_exist path
  | x :: [] ->
      if List.mem_assoc x general_params then
        if !(List.assoc x general_params) then "1"
        else Store.Path.doesnt_exist path
      else Store.Path.doesnt_exist path
...
let rm _t perms path =
  ...
  | x :: [] ->
      if List.mem_assoc x general_params then
        List.assoc x general_params := false
      else Store.Path.doesnt_exist path
  | _ -> Store.Path.doesnt_exist path
