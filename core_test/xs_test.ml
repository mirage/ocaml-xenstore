...
let ( |> ) a b = b a
let id x = x
let unbox = function None -> failwith "unbox" | Some x -> x
...
let op_ids _ =
  ...
  for i = 0 to 100 do
    ...
    match of_int32 i' with None -> () | Some x -> assert (to_int32 x = i')
  done
...
let acl_parser _ =
  ...
  let printer = function None -> "None" | Some x -> "Some " ^ to_string x in
  List.iter
...
let test_packet_parser choose pkt () =
  ...
  while not !finished do
    ...
    | _ ->
        failwith
          (Printf.sprintf "parser failed for %s"
             (pkt |> get_ty |> Op.to_string))
  done
...
let make_example_request op payload tid wire_fmt =
  { op; packet = Xs_protocol.Request.print payload tid 0l; wire_fmt }

let example_request_packets =
  let open Xs_protocol in
  let open Xs_protocol.Request in
  [
    make_example_request Op.Directory
      (PathOp ("/whatever/whenever", Directory))
      5l
  ...
  ; make_example_request Op.Introduce
      (Introduce (4, 5n, 1))
      0l
  ...
  ; make_example_request Op.Watch
      (Watch ("/foo/bar", Xs_protocol.Token.(to_string (of_string "something"))))
      0l
  ...
  ; make_example_request Op.Unwatch
      (Unwatch
         ("/foo/bar", Xs_protocol.Token.(to_string (of_string "somethinglse"))))
      0l
...
let make_example_response op response wire_fmt =
  ...
  { op; packet = Xs_protocol.Response.print response tid rid; wire_fmt }
...
let example_response_packets =
  ...
  ; make_example_response Op.Getperms
      (Getperms Xs_protocol.ACL.{ owner = 2; other = READ; acl = [ (4, NONE) ] })
  ...
  ; make_example_response Op.Directory
      (Directory [ "a"; "b"; "c"; "aseasyas"; "1"; "2"; "3" ])
...
let hexstring x =
  String.concat ""
    ([ "\"" ]
    @ List.map
        (fun i -> Printf.sprintf "\\x%02x" (int_of_char x.[i]))
        (ints 0 (String.length x - 1))
    @ [ "\"" ])
  ...
  let packet_printing =
    ...
    >::: List.map
           (fun example ->
             let description = Xs_protocol.Op.to_string example.op in
             description >:: fun () ->
             assert_equal ~msg:description ~printer:hexstring example.wire_fmt
               (Bytes.to_string @@ Xs_protocol.to_bytes example.packet))
           example_packets
  in
  let suite =
    "xenstore"
