...
let test_implicit_create () =
	...
	let domU = Connection.create (Xs_packet.Domain 1) in
...
let test_setperms_owner () =
	...
	let dom0 = Connection.create (Xs_packet.Domain 0) in
	let dom2 = Connection.create (Xs_packet.Domain 2) in
	let dom5 = Connection.create (Xs_packet.Domain 5) in
	let store = empty_store () in
	let open Xs_packet.Request in
	run store [
		dom0, none, PathOp("/foo", Write ""), OK;
...
let test_restrict () =
	...
	let dom0 = Connection.create (Xs_packet.Domain 0) in
	let dom3 = Connection.create (Xs_packet.Domain 3) in
	let dom7 = Connection.create (Xs_packet.Domain 7) in
	let store = empty_store () in
	let open Xs_packet.Request in
	run store [
