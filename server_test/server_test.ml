...
let failure f reply =
  ...
  | _ ->
      failwith
        (Printf.sprintf "Expected failure, got success: %s"
           (Junk.hexify (Bytes.to_string @@ Xs_protocol.to_bytes reply)))

let list f reply =
  match Xs_protocol.Unmarshal.list reply with
  | Some x -> f x
  | None -> failwith "Failed to unmarshal string list"

let string f reply =
  match Xs_protocol.Unmarshal.string reply with
  | Some x -> f x
  | None -> failwith "Failed to unmarshal string"

let acl f reply =
  match Xs_protocol.Unmarshal.acl reply with
  | Some x -> f x
  | None -> failwith "Failed to unmarshal acl"

let int32 f reply =
  match Xs_protocol.Unmarshal.int32 reply with
  | Some x -> f x
  | None -> failwith "Failed to unmarshal int32"

let equals expected got =
  if expected <> got then
    failwith (Printf.sprintf "Expected %s got %s" expected got)
...
let check_result reply = function
  | OK -> success ignore reply
  | String which -> (success ++ string ++ equals) which reply
  | Err which -> (failure ++ string ++ equals) which reply
  | StringList f -> (success ++ list) f reply
  | Perms f -> (success ++ acl) f reply
  | Tid f -> (success ++ int32) f reply
...
let run store
    (payloads :
      (Connection.t * int32 * Xs_protocol.Request.payload * result) list) =
  List.iter
...
let test_setperms_getperms () =
  ...
  run store
    ...
    ; ( dom0
      ...
      , Perms
          (fun x ->
            assert_equal ~msg:"perms /foo" ~printer:Xs_protocol.ACL.to_string x
              example_acl) )
    ]
...
let test_setperms_owner () =
  ...
  run store
    ...
    ; (* owned by dom5, so dom2 can't setperms *)
      ( dom2
      , none
      , PathOp ("/foo", Setperms { example_acl with Xs_protocol.ACL.owner = 2 })
      , Err "EACCES" )
    ...
    ; (* dom2 sets the owner back to dom5 *)
      ( dom2
      , none
      , PathOp ("/foo", Setperms { example_acl with Xs_protocol.ACL.owner = 5 })
      , OK )
    ]
...
let test_device_create_coalesce () =
  ...
  run store
    [
      ( dom0
      , tid_1
      , PathOp ("/local/domain/0/backend/vbd/1/51712", Write "hello")
      , OK )
    ; ( dom0
      , tid_1
      , PathOp ("/local/domain/1/device/vbd/51712", Write "there")
      , OK )
    ; ( dom0
      , tid_2
      , PathOp ("/local/domain/0/backend/vbd/2/51712", Write "hello")
      , OK )
    ; ( dom0
      , tid_2
      , PathOp ("/local/domain/2/device/vbd/51712", Write "there")
      , OK )
    ; (dom0, tid_1, Transaction_end true, OK)
    ; (dom0, tid_2, Transaction_end true, OK)
    ; ( dom0
      , none
      , PathOp ("/local/domain/0/backend/vbd/1/51712", Read)
      , String "hello" )
    ; ( dom0
      , none
      , PathOp ("/local/domain/0/backend/vbd/2/51712", Read)
      , String "hello" )
    ]
...
let test_transactions_really_do_conflict () =
  ...
  let store = empty_store () in
  let open Xs_protocol.Request in
  run store [ (dom0, none, PathOp ("/a", Mkdir), OK) ];
  let tid = (success ++ int32) id (rpc store dom0 none Transaction_start) in
  run store
    [
...
let assert_watches c expected =
  let got =
    List.rev (Queue.fold (fun acc x -> x :: acc) [] c.Connection.watch_events)
  in
...
let test_watch_event_quota () =
  ...
  run store
    [
      ...
      , none
      , PathOp
          ("/tool/xenstored/quota/number-of-queued-watch-events/1", Write "1")
      , OK )
    ; (dom0, none, PathOp ("/a", Mkdir), OK)
    ; ( dom0
      , none
      , PathOp
          ("/a", Setperms Xs_protocol.ACL.{ owner = 0; other = RDWR; acl = [] })
      , OK )
    ];
  assert_watches dom1 [];
  run store [ (dom1, none, Watch ("/a", "token"), OK) ];
  assert_watches dom1 [ ("/a", "token") ];
  assert_equal ~msg:"nb_dropped_watches" ~printer:string_of_int 0
    dom1.Connection.nb_dropped_watches;
  (* This watch will be dropped *)
  run store [ (dom0, none, PathOp ("/a", Write "hello"), OK) ];
  assert_watches dom1 [ ("/a", "token") ];
  assert_equal ~msg:"nb_dropped_watches" ~printer:string_of_int 1
    dom1.Connection.nb_dropped_watches;
  run store
    [
      ( dom0
      , none
      , PathOp
          ("/tool/xenstored/quota/number-of-queued-watch-events/1", Write "2")
      , OK )
...
let test_transaction_watches () =
  ...
  let tid = (success ++ int32) id (rpc store dom0 none Transaction_start) in
  run store [ (dom0, tid, PathOp ("/a", Write "hello"), OK) ];
  assert_watches dom0 [];
  (* If the transaction is aborted then no watches are generated *)
  run store [ (dom0, tid, Transaction_end false, OK) ];
  assert_watches dom0 [];
...
let test_introduce_watches () =
  ...
  let store = empty_store () in
  let open Xs_protocol.Request in
  run store [ (dom0, none, Watch ("@introduceDomain", "token"), OK) ];
  assert_watches dom0 [ ("@introduceDomain", "token") ];
  Queue.clear dom0.Connection.watch_events;
  assert_watches dom0 [];
  run store [ (dom0, none, Introduce (5, 5n, 5), OK) ];
  assert_watches dom0 [ ("@introduceDomain", "token") ]
...
let _test_quota () =
  ...
  let start = ref 0 in
  let expect n x =
    assert_equal ~msg:"quota" ~printer:string_of_int (!start + n)
      (int_of_string (List.hd x))
  in
  ...
  run store
    ...
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/entries-per-domain/0", Read)
      , StringList (expect 1) )
...
let _test_quota_transaction () =
  ...
  run store
    ...
    ; ( dom0
      ...
      , PathOp
          ( "/local/domain/1"
          , Setperms { example_acl with Xs_protocol.ACL.owner = 1 } )
      , OK )
    ...
    ; ( dom0
      ...
      , PathOp
          ( "/local/domain/2"
          , Setperms { example_acl with Xs_protocol.ACL.owner = 2 } )
      , OK )
    ; (dom1, none, PathOp ("/local/domain/1/data/test", Write ""), OK)
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/entries-per-domain/1", Read)
      , StringList (expect 2) )
    ...
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/entries-per-domain/2", Read)
      , StringList (expect 2) )
    ];
...
let _test_quota_setperms () =
  ...
  let expect quota n x =
    assert_equal ~msg:"quota" ~printer:string_of_int (!quota + n)
      (int_of_string (List.hd x))
  in
  run store
    [
    ...
    ; ( dom0
      ...
      , PathOp
          ( "/local/domain/1"
          , Setperms Xs_protocol.ACL.{ owner = 1; other = NONE; acl = [] } )
      , OK )
    ; (dom1, none, PathOp ("/local/domain/1/private", Mkdir), OK)
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/entries-per-domain/1", Read)
      , StringList (fun x -> dom1_quota := int_of_string (List.hd x)) )
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/entries-per-domain/2", Read)
      , StringList (fun x -> dom2_quota := int_of_string (List.hd x)) )
    ; (dom1, none, PathOp ("/local/domain/1/private/foo", Write "hello"), OK)
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/entries-per-domain/1", Read)
      , StringList (expect dom1_quota 1) )
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/entries-per-domain/2", Read)
      , StringList (expect dom2_quota 0) )
    ; (* Hand this node to domain 2 (who doesn't want it) *)
      ( dom1
      , none
      , PathOp
          ( "/local/domain/1/private/foo"
          , Setperms Xs_protocol.ACL.{ owner = 2; other = NONE; acl = [] } )
      , OK )
...
let test_quota_maxsize () =
  ...
  run store
    [
      ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/default/entry-length", Write "5")
      , OK )
    ; (dom0, none, PathOp ("/a", Write "hello"), OK)
    ; (dom0, none, PathOp ("/a", Write "hello2"), Err "E2BIG")
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/default/entry-length", Write "6")
      , OK )
...
let _test_quota_maxent () =
  ...
  run store
    ...
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/default/number-of-entries", Write "1")
      , OK )
    ; (dom0, none, PathOp ("/a", Write "hello"), Err "EQUOTA")
    ; ( dom0
      , none
      , PathOp ("/tool/xenstored/quota/number-of-entries/0", Write "2")
      , OK )
    ; (dom0, none, PathOp ("/a", Write "hello"), OK)
    ; (dom0, none, PathOp ("/a", Write "there"), OK)
...
let test_control_perms () =
  ...
  run store
    [
      ( dom1
      , none
      , PathOp ("/quota/default/number-of-entries", Write "1")
      , Err "EACCES" )
    ; ( dom1
      , none
      , PathOp ("/tool/xenstored/log/reply-err/ENOENT", Write "1")
      , Err "EACCES" )
    ]
